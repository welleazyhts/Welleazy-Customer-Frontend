...
export const EyeDentalCareAPI = {
        ...
        try {
            ...
            data.forEach((item: any) => {
                const vendorId = (item.vendor && typeof item.vendor === 'object')
                    ? (item.vendor.id || item.vendor.vendor_id)
                    : (item.vendor || item.id || 0);
    ...
    getDoctorAvailability: async (data: { doctor: number | string; date: string }): Promise<any> => {
        try {
            ...
            if (typeof data.doctor === 'object') {
                console.warn("getDoctorAvailability: doctor param is an object, attempting to extract ID", data.doctor);
                const obj = data.doctor as any;
                data.doctor = obj.id || obj.vendor_id || obj.doctor_id || 0;
            }
